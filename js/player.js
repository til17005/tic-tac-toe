...
export default class Player {
  // depth determines ease of game
  constructor(maxDepth = -1) {
    this.maxDepth = maxDepth;
    this.nodesMap = new Map();
  }

  // recursion will stop when a win state is reached or the maxDepth
  getBestMove(board, maximizing = true, callback = () => {}, depth = 0) {
    // clear nodesMap if the function is called for a new move
    if (depth == 0) this.nodesMap.clear();

    // if the board state is terminal (win or full), return the heuristic value
